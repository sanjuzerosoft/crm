...
export class Login {
  ...
  login() {
    ...
    this.authService.login(this.email, this.password).subscribe({
      next: (response) => {
        ...
        if (response.user1) {
          console.log('User Name from API:', response.user1.name);

          this.authService.saveUserName(response.user1.name);

          // Optional: save full user object also
          localStorage.setItem('user', JSON.stringify(response.user1));
        }
  ...
  resetPassword() {
    ...
    this.authService
      .resetPassword(this.resetEmail, this.otp, this.newPassword, this.confirmPassword)
      .subscribe({
        next: (response) => {
